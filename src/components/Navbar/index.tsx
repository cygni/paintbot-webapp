...
const SkipNavigation = styled(RouterLink)`
  position: fixed;
  top: 70px;
  left: -10rem;
  z-index: 1002;
...
function Navbar() {
  ...
  return (
    <>
      <StyledHeader>
        <SkipNavigation to="#content">Skip to content</SkipNavigation>
        <RouterLink to="/">
          <Logo src={logo} alt={'Paint Bot App'} aria-label="Paintbot" />
        </RouterLink>
        <DesktopNav>
          <Nav accountText={accountText} />
        </DesktopNav>
        <HamburgerMenuButton onClick={toggleMenu}>
          <HamburgerIcon src={bars} alt={'menu'} aria-label="Open menu" />
        </HamburgerMenuButton>
      </StyledHeader>
          ...
          <FocusLock returnFocus>
            <JustifyRight>
              <HamburgerMenuButton onClick={closeMenu}>
                <CloseIcon src={times} alt={'close'} aria-label="Close menu" />
              </HamburgerMenuButton>
            </JustifyRight>
